<template>
  ...
  >
    ...
    />
    <AddMusic v-if="addMusicTab" :selected-playlist="selectedPlaylistUuid" @close="addMusicTab=false,getMusic()" />
  </section>
</template>
...
const { data: musics,execute: getMusic } = await useFetch<{ uuid: string; name: string }[]>(
